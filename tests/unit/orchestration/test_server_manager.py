...
def _server_config(**kwargs) -> VllmServerConfig:
    """Build a valid current vLLM config for BaseServerManager tests."""
    return VllmServerConfig(
        hf_model="meta/test-model",
        deploy_config="/tmp/test-deploy.yaml",
        **kwargs,
    )
...
def manager(server_config):
    mgr = FakeServerManager(server_config)
...
class TestBaseServerManager:
    ...
    def test_health_check_success(self, mock_get, manager):
        ...
        mock_get.assert_called_with("http://localhost:8000/v1/models", timeout=5)
    ...
    def test_get_additional_args_dict(self):
        ...
        # Test string (JSON)
        config = _server_config(additional_args='{"key": "value"}')
        manager = FakeServerManager(config)
        args = manager.get_additional_args_dict()
        assert args == {"key": "value"}

        # Test None
        config = _server_config(additional_args=None)
        manager = FakeServerManager(config)
        args = manager.get_additional_args_dict()
        assert args == {}
        ...
        config = _server_config(additional_args='{"invalid": json}')
        manager = FakeServerManager(config)
    ...
    def test_auto_allocation(self):
        ...
        config = _server_config(gpu_ids=None, tensor_parallel_size=2)
        manager = FakeServerManager(config)
    ...
    def test_get_server_logs_reads_last_n_lines(self, manager):
