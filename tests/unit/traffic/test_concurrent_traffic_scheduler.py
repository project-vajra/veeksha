...
def make_scheduler(target: int = 2) -> ConcurrentTrafficScheduler:
    config = ConcurrentTrafficConfig(
        target_concurrent_sessions=target, rampup_seconds=0
    )
...
def test_rampup_measured_from_dispatch_not_construction() -> None:
    ...
    """
    config = ConcurrentTrafficConfig(target_concurrent_sessions=100, rampup_seconds=60)
...
@pytest.mark.unit
def test_concurrent_config_rejects_nonpositive_target() -> None:
    with pytest.raises(ValueError, match="target_concurrent_sessions must be >= 1"):
        ConcurrentTrafficConfig(target_concurrent_sessions=0, rampup_seconds=10)


@pytest.mark.unit
def test_concurrent_config_rejects_negative_rampup() -> None:
    with pytest.raises(ValueError, match="rampup_seconds must be >= 0"):
        ConcurrentTrafficConfig(target_concurrent_sessions=1, rampup_seconds=-1)


@pytest.mark.unit
def test_target_concurrent_sessions_property() -> None:
    scheduler = make_scheduler(target=7)
    assert scheduler.target_concurrent_sessions == 7
...
def test_wait_for_ready_does_not_sleep_past_a_rampup_step() -> None:
    ...
    config = ConcurrentTrafficConfig(target_concurrent_sessions=60, rampup_seconds=60)
    scheduler = ConcurrentTrafficScheduler(config, SeedManager(seed=42))
    scheduler.reset_reference_time()
