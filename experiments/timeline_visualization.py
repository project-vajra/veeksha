...
def plot_timeline(tasks, output_file=None, show_plot=True):
    """
    Plot a timeline visualization similar to the provided example.
    ...
    fig, ax = plt.subplots(figsize=(14, len(tiers) * 1.2))
    ...
    for task in tasks:
        ...
        # Add the task rectangle with hatches and opacity from constants.py
        tier_index = list(colors.keys()).index(tier) if tier in colors else 0
        rect = patches.Rectangle(
            (start, tiers.index(tier) - 0.4),
            duration,
            0.8,
            linewidth=1,
            edgecolor='black',
    ...
    ax.annotate(
        'TTFT',
        xy=(max_end_time / 2, len(tiers) + 0.2),
        xytext=(max_end_time / 2, len(tiers) + 0.6), 
        ha='center',
        va='bottom',
        fontsize=14,
    ...
    # Draw arrows to indicate the iteration span
    # ax.annotate(
    #     '',
    #     xy=(0, len(tiers) + 0.2),
    #     xytext=(max_end_time, len(tiers) + 0.2),
    #     arrowprops=dict(arrowstyle='<->', lw=1.5)
    # )
    ...
    if output_file:
        ...
        plt.savefig("chunk_timeline.pdf", dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    # Plot the timeline
    plot_timeline(tasks, output_file="timeline_visualization.png")
    
    print(f"Timeline visualization created and saved as 'timeline_visualization.png'")
