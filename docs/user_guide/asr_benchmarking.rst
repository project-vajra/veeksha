...
Nova, ElevenLabs Scribe, Mistral Voxtral, Cartesia Ink, or Together wire
adapter; it does not select a different client lifecycle.
...
* ``provider``: normalized serving provider family, such as ``vajra``, ``vllm``,
  ``deepgram``, ``elevenlabs``, ``mistral``, ``cartesia``, or ``together``.
...
* ``provider_protocol``: concrete wire protocol selected by
  ...
  ``mistral_realtime_transcription``, ``cartesia_stt_websocket_manual``, or
