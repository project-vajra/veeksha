...
if TYPE_CHECKING:
    from veeksha.new.config.client import BaseClientConfig
...
class OpenAICompletionsClient(OpenAIBaseClient):
    ...
    async def send_request(
        ...
        if isinstance(request.metadata, dict):
            api_mode = request.metadata.get("api_mode")
            if api_mode not in (None, "completions"):
                logger.warning(
                    "OpenAICompletionsClient received api_mode=%r for request_id=%s; "
                    "this client will still use /completions. If you need per-request "
                    "routing (chat + completions in one run), use client.type=openai_router.",
                    api_mode,
                    request.id,
                )
