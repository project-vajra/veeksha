...
def _run_benchmarks_for_endpoint(configs_for_endpoint: List[BenchmarkConfig]):
    ...
    if len(configs_for_endpoint) > 1:
        logger.info(
            f"Running {len(configs_for_endpoint)} benchmark configurations sequentially for endpoint '{endpoint_name}'"
        )
...
if __name__ == "__main__":
    ...
    num_endpoints = len(grouped_configs)

    # Check if dashboard is enabled
    has_dashboard_enabled = any(bc.dashboard_config.enabled for bc in benchmark_configs)

    if has_dashboard_enabled and num_endpoints > 1:
        logger.warning(
            f"Dashboard is enabled with {num_endpoints} endpoints. "
            f"Parallel execution is disabled when dashboard is enabled. "
            f"Runs will be sequential."
        )
    ...
    try:
        ...
        if has_dashboard_enabled and len(benchmark_configs) == 1:
            ...
            # No dashboard enabled - use parallel execution
            num_parallel_jobs = num_endpoints
            max_parallel_runs = benchmark_configs[0].max_parallel_runs
            max_workers = (
                max_parallel_runs
                if max_parallel_runs is not None
                else num_parallel_jobs
            )
                ...
                with ProcessPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in as_completed(future_to_endpoint):
                        ...
                        try:
                            ...
                            logger.exception(
                                f"Endpoint '{endpoint}' generated an exception"
                            )
                            all_succeeded = False
