...
class TTFTGraph(LiveGraph):
    ...
    def update_plot(self):
        ...
        perf_indicator = (
            "🟢 Fast" if avg_val < 100 else "🟡 OK" if avg_val < 300 else "🔴 Slow"
        )
        ...
        plt.scatter(recent_times, recent_points, marker="dot", color="cyan")
        plt.plotsize(68, 12)
        # Ensure x-axis reflects elapsed seconds and updates each refresh
        elapsed = max(1e-6, time_module.time() - self.start_time)
        plt.xlim(0.0, elapsed)
        # Set explicit second ticks so labels show seconds, not samples
        num_ticks = 6
        tick_step = elapsed / max(1, (num_ticks - 1))
        tick_positions = [i * tick_step for i in range(num_ticks)]
        fmt = (lambda v: f"{v:.1f}") if elapsed < 10 else (lambda v: f"{int(round(v))}")
        tick_labels = [fmt(v) for v in tick_positions]
        try:
            plt.xticks(tick_positions, tick_labels)
        except Exception:
            pass
        plt.title(
            f"TTFT - {status} | Latest: {latest_val:.1f}ms, Avg: {avg_val:.1f}ms {perf_indicator}"
        )
        plt.xlabel("Time (s)")
        plt.ylabel("TTFT (ms)")
        plt.grid(True, True)
...
class TPOTGraph(LiveGraph):
    ...
    def update_plot(self):
        ...
        if not PLOTEXT_AVAILABLE:
            self.update(
                "📉 [bold green]Time per Output Token[/bold green]\n\n[dim]Install plotext: pip install plotext[/dim]"
            )
            return
        ...
        latest_val = recent_points[-1]
        perf_indicator = (
            "🟢 Fast" if avg_val < 10 else "🟡 OK" if avg_val < 50 else "🔴 Slow"
        )
        ...
        plt.scatter(recent_times, recent_points, marker="dot", color="green")
        plt.plotsize(68, 12)
        # Ensure x-axis reflects elapsed seconds and updates each refresh
        elapsed = max(1e-6, time_module.time() - self.start_time)
        plt.xlim(0.0, elapsed)
        ...
        try:
            ...
            pass
        plt.title(
            f"TPOT - {status} | Latest: {latest_val:.1f}ms, Avg: {avg_val:.1f}ms {perf_indicator}"
        )
        plt.xlabel("Time (s)")
        plt.ylabel("TPOT (ms)")
        plt.grid(True, True)
...
class TBTGraph(LiveGraph):
    ...
    def update_plot(self):
        ...
        if not PLOTEXT_AVAILABLE:
            self.update(
                "⏱️ [bold yellow]Time Between Tokens[/bold yellow]\n\n[dim]Install plotext: pip install plotext[/dim]"
            )
            return
        ...
        plt.scatter(recent_times, recent_points, marker="dot", color="yellow")
        plt.plotsize(68, 12)
        # Ensure x-axis reflects elapsed seconds and updates each refresh
        elapsed = max(1e-6, time_module.time() - self.start_time)
        plt.xlim(0.0, elapsed)
        ...
        try:
            ...
            pass
        plt.title(
            f"TBT - {status} | Latest: {latest_val:.1f}ms, Avg: {avg_val:.1f}ms {perf_indicator}"
        )
        plt.xlabel("Time (s)")
        plt.ylabel("TBT (ms)")
        plt.grid(True, True)
...
class ThroughputGraph(LiveGraph):
    ...
    def update_plot(self):
        ...
        if not PLOTEXT_AVAILABLE:
            self.update(
                "🚀 [bold yellow]Throughput (QPS)[/bold yellow]\n\n[dim]Install plotext: pip install plotext[/dim]"
            )
            return
        ...
        perf_indicator = (
            "🟢 High" if avg_val > 2.0 else "🟡 OK" if avg_val > 0.5 else "🔴 Low"
        )
        ...
        plt.scatter(recent_times, recent_qps, marker="dot")
        plt.plotsize(78, 13)
        # Ensure x-axis reflects elapsed seconds and updates each refresh
        elapsed = max(1e-6, time_module.time() - self.start_time)
        plt.xlim(0.0, elapsed)
        ...
        try:
            ...
            pass
        plt.title(
            f"Throughput (QPS) - {status} | Current: {latest_val:.2f}, Avg: {avg_val:.2f} {perf_indicator}"
        )
...
class LatencyOverTime(LiveGraph):
    ...
    def update_plot(self):
        ...
        if not PLOTEXT_AVAILABLE:
            self.update(
                "📊 [bold magenta]End-to-End Latency[/bold magenta]\n\n[dim]Install plotext: pip install plotext[/dim]"
            )
            return
        ...
        latest_val = recent_points[-1]
        perf_indicator = (
            "🟢 Fast" if avg_val < 500 else "🟡 OK" if avg_val < 2000 else "🔴 Slow"
        )
        ...
        plt.scatter(recent_times, recent_points, marker="dot", color="magenta")
        plt.plotsize(68, 12)
        # Ensure x-axis reflects elapsed seconds and updates each refresh
        elapsed = max(1e-6, time_module.time() - self.start_time)
        plt.xlim(0.0, elapsed)
        plt.title(
            f"Latency (ms) - {status} | Latest: {latest_val:.0f}, Avg: {avg_val:.0f} {perf_indicator}"
        )
        plt.xlabel("Time (s)")
...
class VeekshaDashboard(App):
    ...
    def on_mount(self) -> None:
        ...
        self.notify(
            "📊 Analytics Dashboard started! 'p'=pause, 'r'=resume, 'n/b'=switch benchmark, 'q'=quit",
            timeout=5,
        )
    ...
    def action_next_benchmark(self) -> None:
        ...
        self.update_subtitle()
        self.notify(
            f"📊 Switched to benchmark: {benchmark_ids[next_idx]}",
    ...
    def action_prev_benchmark(self) -> None:
        ...
        self.update_subtitle()
        self.notify(
            f"📊 Switched to benchmark: {benchmark_ids[prev_idx]}",
            severity="information",
        )
    ...
    def action_save_analysis(self) -> None:
        ...
        if self.benchmark_completed:
            self.notify(
                "💾 Analysis saved to benchmark results directory",
                severity="information",
            )
        else:
            self.notify(
                "⚠️ Benchmark still running - analysis will be available after completion",
                severity="warning",
            )
    ...
    def action_quit(self) -> None:
        ...
        if self.benchmark_completed or self.analysis_mode:
            ...
            self.notify(
                "⚠️ Benchmark still running! Press 'q' again to force quit or wait for completion",
                severity="warning",
            )
    ...
    def mark_benchmark_completed(self) -> None:
        ...
        self.final_requests = (
            self.dashboard_state.get_all_requests()
        )  # Get both live and completed

        # Stop updating graphs but keep final data visible
        self.freeze_graphs()

        self.notify(
            "✅ Benchmark completed! Analysis mode active. Data preserved for review.",
            severity="success",
        )
    ...
    def update_graphs(self) -> None:
        ...
        if aggregate_stats.recent_ttft_ms:
            ...
            if len(aggregate_stats.recent_ttft_ms) > last_count:
                ...
                # Redraw to advance time axis even without new points
                self.ttft_graph.update_plot()
...
"""
    ...
    def update_benchmark_status(self) -> None:
        ...
        success_rate = (
            aggregate_stats.completed_count / max(aggregate_stats.total_requests, 1)
        ) * 100

        # Status indicator
        status_emoji = "🟢" if not self.benchmark_completed else "🔵"
        status_text_indicator = (
            "RUNNING" if not self.benchmark_completed else "COMPLETED"
        )
...
🚀 [bold]QPS:[/] {qps_display}
    ...
    def update_metrics_display(self) -> None:
        ...
        ttft_indicator = (
            "🟢"
            if aggregate_stats.avg_ttft_ms < 100
            else "🟡" if aggregate_stats.avg_ttft_ms < 500 else "🔴"
        )
        tpot_indicator = (
            "🟢"
            if aggregate_stats.avg_tpot_ms < 10
            else "🟡" if aggregate_stats.avg_tpot_ms < 50 else "🔴"
        )
        latency_indicator = (
            "🟢"
            if aggregate_stats.avg_latency_ms < 500
            else "🟡" if aggregate_stats.avg_latency_ms < 2000 else "🔴"
        )
...
🎯 [bold]Throughput:[/] {aggregate_stats.completed_count/max(duration,1):.1f}/s
    ...
    def update_live_requests(self) -> None:
        ...
        sorted_requests = sorted(
            live_requests, key=lambda x: x.start_timestamp, reverse=True
        )
        ...
        for req in sorted_requests[:10]:  # Show only top 10 to avoid clutter
            ...
            display_id = (
                request_id_str[:12] if len(request_id_str) > 12 else request_id_str
            )
...
def run_dashboard_frontend(
    ...
    logger.info(
        "Dashboard: Starting console metrics display (use run_dashboard_with_benchmark for TUI)"
    )
...
def run_dashboard_with_benchmark(benchmark_config) -> None:
    ...
    class BenchmarkDashboard(VeekshaDashboard):
        ...
        def check_benchmark_status(self) -> None:
            ...
            if benchmark_complete.is_set():
                if not self.benchmark_completed:  # Only do this once
                    if benchmark_error:
                        self.notify(
                            f"Benchmark failed: {benchmark_error}", severity="error"
                        )
                        self.analysis_mode = True
                    else:
        ...
        def action_quit(self) -> None:
            ...
            if self.benchmark_completed or self.analysis_mode:
                self.notify(
                    "👋 Goodbye! Analysis data has been preserved.",
                    severity="information",
                )
                self.exit()
            else:
                ...
                if not hasattr(self, "_quit_warned"):
                    ...
                    self.notify(
                        "⚠️ Benchmark still running! Press 'q' again to force quit",
                        severity="warning",
                    )
                else:
