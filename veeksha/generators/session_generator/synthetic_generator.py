...
from veeksha.generators.interval_generator.generator_registry import RequestIntervalGeneratorRegistry
...
class SyntheticSessionGenerator(BaseSessionGenerator):
    ...
    def rejection_sample(
        ...
        next_interval = self.session_interval_generator.get_next_inter_request_time() * 1000 # todo: ms or s?
        ...
        for request in session:
            ...
            request['timestamp'] = current_timestamp + (request['timestamp'] - session_original_timestamp)
    ...
    def save_requests_as_trace(self, requests_df: pd.DataFrame):
        ...
        def create_clean_filename():
            base_name = self.config.trace_file_name if self.config.trace_file_name else "session_trace"
            ...
            if isinstance(interval_config, PoissonRequestIntervalGeneratorConfig or GammaRequestIntervalGeneratorConfig):
    ...
    def find_sessions(self, requests):
        ...
        for idx, request in enumerate(requests):
            ...
            if best_match_length > self.config.minimum_prefix_match * len(current_hashes):
        ...
        MAX_SESSION_GAP = self.config.max_request_interval * 1000  # Convert to milliseconds
    ...
    def sample_sessions(self, sessions):
        ...
        while remaining_sessions:
            ...
            session, timestamp = self.rejection_sample(remaining_sessions, timestamp, current_seed)
    ...
    def generate_sessions(self, requests_df: pd.DataFrame) -> pd.DataFrame:
        ...
        requests_df['hash_ids'] = requests_df.apply(lambda row: self.create_running_hashes(row['original_hash_ids']), axis=1)
        ...
        logger.info(f"Deleted {sessions_to_delete_lt_min} sessions with less than {self.config.min_session_size} requests")
        ...
        sessions_to_delete = [session_id for session_id, session in sessions.items() if len(session) > self.config.max_session_size]
        for session_id in sessions_to_delete:
            del sessions[session_id]
        
        if sessions_to_delete:
            logger.info(f"Deleted {len(sessions_to_delete)} sessions with more than {self.config.max_session_size} requests")
        ...
        for request in sampled_requests:
            ...
            request['prefix_match_pct'] = round(100*(request['prefix_match_n_hashes'] / len(request['hash_ids'])), 2) if request['hash_ids'] else 0
            prefix_cache.add_request(request)

            total_hashes_matched += request['prefix_match_n_hashes']
            total_hashes_seen += len(request['hash_ids'])

            cummulative_prefix_match_pct = round(100*(total_hashes_matched / total_hashes_seen), 2) if total_hashes_seen > 0 else 0
