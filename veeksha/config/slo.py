...
class ConstantSloConfig(BaseSloConfig):
    ...
    metric: SloMetric = field(
        default=SloMetric.TTFT, metadata={"help": "The metric this SLO applies to"}
    )
...
class DeadlineSloConfig(BaseSloConfig):
    ...
    ttft_threshold: Optional[float] = field(
        default=0.1,
        metadata={
            "help": "Fixed TTFT threshold in seconds. If None, uses prediction-based threshold."
        },
    )
    tbt_threshold: float = field(
        default=0.03, metadata={"help": "TBT threshold in seconds"}
    )
    ttft_prediction_type: Optional[Literal["offset", "multiplier"]] = field(
        default=None,
        metadata={
            "help": "Type of prediction-based TTFT threshold. If None, uses ttft_threshold."
        },
    )
    ttft_prediction_value: Optional[float] = field(
        default=None,
        metadata={
            "help": "Value for prediction-based TTFT threshold (offset or multiplier)"
        },
    )
    ttft_min_value: Optional[float] = field(
        default=None,
        metadata={"help": "Minimum value for TTFT threshold (for clamping)"},
    )
    ttft_max_value: Optional[float] = field(
        default=None,
        metadata={"help": "Maximum value for TTFT threshold (for clamping)"},
    )
