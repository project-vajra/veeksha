...
class SimpleMetricSlo:
    ...
    def _extract_metric_values(self, request_metrics: Dict[str, Any]) -> List[float]:
        ...
        if (
            self.config.metric == SloMetric.TBT
            and values
    ...
    def evaluate(
        ...
        threshold = self.get_threshold(
            predictions=predictions, request_metrics=request_metrics
        )
...
class PredictionBasedSlo(SimpleMetricSlo):
    ...
    def get_threshold(self, **kwargs: Any) -> float:
        ...
        if not predictions or not request_metrics:
            raise ValueError(
                "Prediction-based SLOs require both predictions and request_metrics"
            )
        ...
        if not predictor_values:
            raise ValueError(
                f"No values found for predictor field {self.config.predictor_field}"
            )
        ...
        request_value = (
            ...
            else predictor_values
        )
        return self._calculate_threshold(predictions, request_value)

    def _calculate_threshold(
        self, predictions: Dict[int, float], request_value: float
    ) -> float:
...
class DeadlineSlo:
    ...
    def evaluate(
        ...
        for i, ttft in enumerate(ttft_values):
            ...
            request_tbt_start = (
                i * (len(tbt_flat) // total_requests) if total_requests > 0 else 0
            )
            request_tbt_end = (
                (i + 1) * (len(tbt_flat) // total_requests)
                if total_requests > 0
                else len(tbt_flat)
            )

            request_tbt_values = (
                tbt_flat[request_tbt_start:request_tbt_end]
                if request_tbt_start < len(tbt_flat)
                else []
            )
        ...
        deadline_miss_rate = (
            missed_deadlines / total_requests if total_requests > 0 else 1.0
        )
